...
__all__ = ('Snowflake', 'SnowflakeGenerator', 'get_uuid')
...
gen = SnowflakeGenerator(INSTANCE_VALUE)
for i in range(100):
    val = next(gen)
    print(val)
"""
...
INSTANCE_VALUE = abs(hash(f'{os.getpid()}{os.getenv("HOSTNAME", 1)}')) % MAX_INSTANCE
gen = SnowflakeGenerator(INSTANCE_VALUE)
